...
API retrieves data related to ethnic distribution for given planning area name and year. If gender is not specified, then result set would contain figures for both genders, male and female.
...
# Function to return Ethnic status based on gender, year and area.
def retrieve_ethnic_status(current_url, authorisation_string):
    ...
    if isinstance(response_retrieve, list):
        ...
        chinese = response_dictionary["chinese"]
        malays = response_dictionary["malays"]
        indian = response_dictionary["indian"]
...
def main():
    ...
    base_url = "https://www.onemap.gov.sg/api/public/popapi/getEthnicGroup" # ?planningArea=Bedok&year=2010&gender=male   
    print("code run ok")
    ...
    try:
        ...
        ethnic_status_df = pd.DataFrame(columns=["planning_area", "chinese","malays","indian","others","year","gender"])
        ...
        for area in planning_area_unique:
            for year in year_list:
                ...
                retrieve_ethnic_status_female_df = retrieve_ethnic_status(custom_url_female, authorisation_string)
                ethnic_status_df = pd.concat([ethnic_status_df, retrieve_ethnic_status_female_df], axis=0, ignore_index=True)
                
                # Retrieve information by year, area and gender as male to concatenate
                rerieve_ethnic_status_male_df = retrieve_ethnic_status(custom_url_male, authorisation_string)
                ethnic_status_df = pd.concat([ethnic_status_df, rerieve_ethnic_status_male_df], axis=0, ignore_index=True)


        
        # Export Joined Dataset and Reimport to Check
        export_filepath = "../assets/onemap/population_ethnic_status"
        export_file_pickle_csv_check(ethnic_status_df,export_filepath)
        
        print("COMPLETE:", ethnic_status_df.info())
